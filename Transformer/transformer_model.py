...
from custom_layer import ReLU, Three_Linear_One_Input, Three_Linear_One_Input_ABA, Three_Linear_One_Input_ASA, Three_Linear_Two_Input, Three_Linear_Two_Input_ABA, Three_Linear_Two_Input_ASA

class Transformer(nn.Module):

    def __init__(self, model_dimension, src_vocab_size, trg_vocab_size, number_of_heads, number_of_layers, dropout_probability, log_attention_weights=False, learning_rule = 'bp', gcp = False, get_li = False):
        super().__init__()
        ...
        #mha = MultiHeadedAttention(model_dimension, number_of_heads, dropout_probability, log_attention_weights, learning_rule=learning_rule, sign_mha = sign_mha, gcp = gcp)
        
        mha3 = MultiHeadedAttention_Merge3(model_dimension, number_of_heads, dropout_probability, log_attention_weights, learning_rule = learning_rule, gcp = gcp, get_li=get_li)
        mha2 = MultiHeadedAttention_Merge2(model_dimension, number_of_heads, dropout_probability, log_attention_weights, learning_rule = learning_rule, gcp = gcp, get_li=get_li)
        
        pwn = PositionwiseFeedForwardNet(model_dimension, dropout_probability, 4, learning_rule = learning_rule, get_li=get_li)
        encoder_layer = EncoderLayer(model_dimension, dropout_probability, mha3, pwn)
        decoder_layer = DecoderLayer(model_dimension, dropout_probability, mha3, mha2, pwn)
    ...
    def forward(self, src_token_ids_batch, trg_token_ids_batch, src_mask, trg_mask):
        ...
        if self.training and self.get_li:
            for m in self.modules():
              if isinstance(m, Linear_ABA) or isinstance(m, Three_Linear_One_Input_ABA) or isinstance(m, Three_Linear_Two_Input_ABA)\
                  or isinstance(m, Linear_ASA) or isinstance(m, Three_Linear_One_Input_ASA) or isinstance(m, Three_Linear_Two_Input_ASA):  
                  li += m.li
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, src_representations_batch, src_mask):
        ...
        encoder_self_attention = lambda srb: self.multi_headed_attention(qkv=srb, mask=src_mask)
...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, model_dimension, dropout_probability, multi_headed_attention3, multi_headed_attention2, pointwise_net):
        super().__init__()
        ...
        self.trg_multi_headed_attention = copy.deepcopy(multi_headed_attention3)
        self.src_multi_headed_attention = copy.deepcopy(multi_headed_attention2)
        self.pointwise_net = pointwise_net
    ...
    def forward(self, trg_representations_batch, src_representations_batch, trg_mask, src_mask):
        ...
        decoder_trg_self_attention = lambda trb: self.trg_multi_headed_attention(qkv=trb, mask=trg_mask)
        decoder_src_attention = lambda trb: self.src_multi_headed_attention(query=trb, kv=srb, mask=src_mask)
...
class PositionwiseFeedForwardNet(nn.Module):
    ...
    def __init__(self, model_dimension, dropout_probability, width_mult=4, learning_rule = 'bp', get_li=False):
        ...
        '''
        if learning_rule == 'sign':
            self.linear1 = Linear_Fake(model_dimension, width_mult * model_dimension)
            self.linear2 = Linear_Fake(width_mult * model_dimension, model_dimension, zero2one = True)
        elif learning_rule == 'ifc':
            self.linear2 = Inplace_FC(width_mult * model_dimension, model_dimension)
            #self.linear2 = BiasFC(width_mult * model_dimension, model_dimension)
        '''
        if learning_rule == 'aba1':
            self.linear2 = Linear_ABA(width_mult * model_dimension, model_dimension, get_li = get_li)
        elif learning_rule == 'aba2':
            self.linear2 = Linear_ABA(width_mult * model_dimension, model_dimension, get_li=get_li)
            self.linear1 = Linear_ABA(model_dimension, width_mult * model_dimension, get_li = get_li)
        elif learning_rule == 'aba3' or learning_rule == 'aba4':
            self.linear2 = Linear_ABA(width_mult * model_dimension, model_dimension, get_li=get_li)
            self.linear1 = Linear_ABA(model_dimension, width_mult * model_dimension, get_li=get_li)
        
        if learning_rule == 'asa1':
            self.linear2 = Linear_ASA(width_mult * model_dimension, model_dimension, relu=True, get_li=get_li)
...
class MultiHeadedAttention_Merge3(nn.Module):
    """
    ...
    def __init__(self, model_dimension, number_of_heads, dropout_probability, log_attention_weights, learning_rule = 'bp', get_li = False, gcp = False):
        super().__init__()
        ...
        self.qkv_nets = Three_Linear_One_Input(model_dimension, model_dimension)  # identity activation hence "nets"
        self.self_attention = Self_Attention(self.head_dimension, dropout_probability)
        self.out_projection_net = nn.Linear(model_dimension, model_dimension)
        if learning_rule == 'aba3':
            self.out_projection_net = Linear_ABA(model_dimension, model_dimension, get_li=get_li)
        elif learning_rule == 'aba4':
            self.out_projection_net = Linear_ABA(model_dimension, model_dimension, get_li=get_li)
            self.qkv_nets = Three_Linear_One_Input_ABA(model_dimension, model_dimension, get_li=get_li)  # identity activation hence "nets"
        ...
        if learning_rule == 'asa3':
            ...
            self.out_projection_net = Linear_ASA(model_dimension, model_dimension, get_li=get_li)
            self.qkv_nets = Three_Linear_One_Input_ASA(model_dimension, model_dimension, get_li=get_li)  # identity activation hence "nets"
    ...
    def forward(self, qkv, mask):
        ...
        query, key, value = self.qkv_nets(qkv)
...
class MultiHeadedAttention_Merge2(nn.Module):
    """
        This module already exists in PyTorch. The reason I implemented it here from scratch is that
        PyTorch implementation is super complicated as they made it as generic/robust as possible whereas
        on the other hand I only want to support a limited use-case.
    ...
    def __init__(self, model_dimension, number_of_heads, dropout_probability, log_attention_weights, learning_rule = 'bp', get_li = False, gcp = False):
        ...
        #self.qkv_nets = get_clones(nn.Linear(model_dimension, model_dimension), 3)  # identity activation hence "nets"
        self.qkv_nets = Three_Linear_Two_Input(model_dimension, model_dimension)
        self.self_attention = Self_Attention(self.head_dimension, dropout_probability)
        self.out_projection_net = nn.Linear(model_dimension, model_dimension)
        if learning_rule == 'aba3':
            self.out_projection_net = Linear_ABA(model_dimension, model_dimension, get_li=get_li)
            ...
            self.out_projection_net = Linear_ABA(model_dimension, model_dimension, get_li=get_li)
            self.qkv_nets = Three_Linear_Two_Input_ABA(model_dimension, model_dimension, get_li=get_li)  # identity activation hence "nets"
    
        elif learning_rule == 'asa3':
            self.out_projection_net = Linear_ASA(model_dimension, model_dimension, get_li=get_li)
        elif learning_rule == 'asa4':
            self.out_projection_net = Linear_ASA(model_dimension, model_dimension, get_li=get_li)
            self.qkv_nets = Three_Linear_Two_Input_ASA(model_dimension, model_dimension, get_li=get_li)  # identity activation hence "nets"
    ...
    def forward(self, query, kv, mask):
        ...
        #query, key, value = [net(x).view(batch_size, -1, self.number_of_heads, self.head_dimension).transpose(1, 2)
        #                     for net, x in zip(self.qkv_nets, (query, key, value))]
        query, key, value = self.qkv_nets(query, kv)
        query = query.view(batch_size, -1, self.number_of_heads, self.head_dimension).transpose(1, 2)
        key = key.view(batch_size, -1, self.number_of_heads, self.head_dimension).transpose(1, 2)
        value = value.view(batch_size, -1, self.number_of_heads, self.head_dimension).transpose(1, 2)
...
class Self_Attention(nn.Module):
    def __init__(self, head_dimension, dropout_probability):
