...
from Auxiliary_Activation_Learning import Conv2d_ARA, Conv2d_ABA, Conv2d_ASA, Distribute_ARA
from custom_layer import BatchNorm2d, ReLU, BnReLUConv, BnReLUConvBn, ConvBn_ARA
...
class ResNet_ARA(nn.Module):
    def __init__(self, dataset='cifar10', model = 'resnet18', ARA_stride = [2,2,2,2]):
        ...
        if model == 'resnet18' or model == 'resnet34':
            self.layer1 = self._make_layer(block, 64, num_blocks[0], stride=1, k = ARA_stride[0])
        else:
            self.layer0 = ARA_Bottleneck(64, 64, 1 , True)
            # self.layer0 is first block with 64 channel dimensions.
            # Although it is ARA_Bottleneck_GCP, they do backpropagation because ARA_Conv uses actual input activation as out=ARA_Conv(x, ABA) and x=ABA.
            ...
            self.layer1 = self._make_layer(block, 64, num_blocks[0]-1, stride=1, k = ARA_stride[0])
            
        self.layer2 = self._make_layer(block, 128, num_blocks[1], stride=1, k = ARA_stride[1])
        self.layer3 = self._make_layer(block, 256, num_blocks[2], stride=1, k = ARA_stride[2])
        self.layer4 = self._make_layer(block, 512, num_blocks[3], stride=1, k = ARA_stride[3])
        self.classifier = nn.Sequential(
    ...
    def _make_layer(self, block, planes, num_blocks, stride, k = 2):
        ...
        for i, stride in enumerate(strides):
            ...
            if i != num_blocks-1:
                layers.append(block(self.in_planes, planes, stride, distribute = distribute))
                self.in_planes = planes * block.expansion
            else:
                if planes != 512:
                    layers.append(block(self.in_planes, 2 * planes, stride, distribute = distribute))
                    self.in_planes = 2 * planes * block.expansion
                else:
                    layers.append(block(self.in_planes, planes, 1, distribute = distribute))
                    self.in_planes = planes * block.expansion
...
class ResNet_ABA_ASA_GCP(nn.Module):
    def __init__(self, dataset='cifar10', model = 'resnet18', learning_rule='aba', ARA_stride = [2,2,2,2], get_li=False):
        super(ResNet_ABA_ASA_GCP, self).__init__()
        self.model = model
        self.dataset = dataset
        block = ABA_ASA_BasicBlock
        # Choose dimension of architecture by dataset
        if dataset == 'mnist':
            num_classes = 10
        elif dataset == 'cifar10' or dataset == 'svhn':
    ...
    def _make_layer(self, block, planes, num_blocks, stride, k = 2, learning_rule = 'aba', get_li=False):
        strides = [1]*(num_blocks-1) + [2] # ABA_ASA_Basick block includes stride = 2 last order
        layers = []
        for i, stride in enumerate(strides):
            if (i) % k == 0:
                bp_conv = True    
    ...
    def forward(self, x):
        ...
        if self.get_li and self.training:
            for m in self.modules():
              if isinstance(m, Conv2d_ABA) or isinstance(m, Conv2d_ASA):  
                  if li==None:
                      li = m.li
                  else:
                      if m.li != None:
...
class ARA_BasicBlock(nn.Module):
    expansion = 1
    def __init__(self, in_planes, planes, stride=1, distribute = False):
...
class ABA_ASA_BasicBlock(nn.Module):
    expansion = 1
    def __init__(self, in_planes, planes, stride=1, learning_rule='aba', bp_conv= False, get_li=False):
        super(ABA_ASA_BasicBlock, self).__init__()
        self.conv1 = Conv2d_ABA(in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False, get_li=get_li)
        if learning_rule=='asa':
        ...
        self.bn2 = BatchNorm2d(planes)  
        if stride != 1 or in_planes != self.expansion*planes:
            self.short_conv = Conv2d_ABA(in_planes, self.expansion*planes, kernel_size=1, stride=stride, bias=False, get_li=get_li)
            if learning_rule=='asa':
...
class ABA_ASA_Bottleneck(nn.Module):
    expansion = 4
    def __init__(self, in_planes, planes, stride=1, learning_rule='aba', bp_conv=False, get_li=False):
        super(ABA_ASA_Bottleneck, self).__init__()
        self.conv1 = Conv2d_ABA(in_planes, planes, kernel_size=1, bias=False, get_li=get_li)
        if learning_rule == 'asa':
        ...
        self.shortcut = nn.Sequential()
        if stride != 1 or in_planes != self.expansion*planes:
            self.short_conv = Conv2d_ABA(in_planes, self.expansion*planes,
                          kernel_size=1, stride=stride, bias=False, get_li=get_li)
            if learning_rule == 'asa':
